...
import { AISuggestions, Customer, CompanyAnalysis, InvoiceData, AIJournalSuggestion, User, ApplicationCode, Estimate, EstimateLineItem, ApprovalRoute, Job, LeadStatus, JournalEntry, Application, ApplicationWithDetails, CompanyInvestigation, CustomProposalContent, LeadProposalPackage, MarketResearchReport, EstimateDraft, ExtractedParty, GeneratedEmailContent, UUID, Project, AllocationDivision, AccountItem, Lead } from '../types.ts'; // Correctly import Lead
...
export const generateLeadReplyEmail = async (lead: Lead): Promise<GeneratedEmailContent> => {
    ...
    return withRetry(async () => {
        const ai = getAiClient();
        // Use inquiryTypes array for the prompt
        const inquiryContent = (lead.inquiryTypes && lead.inquiryTypes.length > 0)
            ? `問い合わせ種別: ${lead.inquiryTypes.join(', ')}`
            : '問い合わせ種別: 記載なし';
...
export const analyzeLeadData = async (leads: Lead[]): Promise<string> => {
    ...
    return withRetry(async () => {
        ...
        データサンプル:
        ${JSON.stringify(leads.slice(0, 3).map(l => ({ company: l.company, status: l.status, inquiryType: l.inquiryTypes?.join(', ') || (l.inquiryType || ''), message: l.message })), null, 2)}
        `;
        const response = await ai.models.generateContent({ model, contents: prompt, config: { } });
