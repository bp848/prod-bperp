...
// FIX: Helper to format Supabase errors for console and UI - this is correct
const formatSupabaseError = (entityName: string, error: any): string => {
  ...
  if (error instanceof TypeError && (error.message.includes('Failed to fetch') || error.message.includes('Network request failed'))) {
    ...
    // FIX: Safely access error.message, error.details, error.hint, error.code, error.error_description
    const supabaseErrorMessage = error.message || error.details || error.hint || error.code || error.error_description;
  ...
  console.error(`Error fetching ${entityName}:`, { 
    ...
    stack: error?.stack,
    originalError: error,
    error_description: error?.error_description // FIX: Add for debugging and consistent error access
  });
  return message;
};
...
export const isSupabaseUnavailableError = (error: any): boolean => {
  ...
  const message = typeof error === 'string' ? error : error.message || error.error_description || String(error); // Ensure message is a string for regex test
...
// FIX: Apply formatSupabaseError consistently to all functions in dataService.ts
...
export const getLeads = async (): Promise<Lead[]> => {
    try {
        ...
        return data.map((d: any) => ({ 
            ...
            isFirstVisit: d.is_first_visit, // Map directly to boolean
            visitCount: d.visit_count // Map directly to number
        }));
    } catch (error: any) {
