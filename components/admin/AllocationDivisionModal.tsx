...
import { AllocationDivision } from '../../types';
import { Loader, Save, X } from '../Icons.tsx';

interface AllocationDivisionModalProps {
  item: AllocationDivision | null;
  onClose: () => void;
  onSave: (item: Partial<AllocationDivision>) => Promise<void>;
}

const AllocationDivisionModal: React.FC<AllocationDivisionModalProps> = ({ item, onClose, onSave }) => {
  const [formData, setFormData] = useState<Partial<AllocationDivision>>(item || { name: '', isActive: true });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[60] p-4">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold">{item ? '振分区分 編集' : '振分区分 作成'}</h2>
...
export default AllocationDivisionModal;