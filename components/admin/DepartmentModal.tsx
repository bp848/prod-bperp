...
interface DepartmentModalProps {
  ...
  onClose: () => void;
  onSave: (item: Partial<Department>) => Promise<void>;
}

const DepartmentModal: React.FC<DepartmentModalProps> = ({ item, onClose, onSave }) => {
  const [formData, setFormData] = useState<Partial<Department>>(item || { name: '' });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[60] p-4">
      <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold">{item ? '部署 編集' : '部署 作成'}</h2>
          <button type="button" onClick={onClose}><X className="w-6 h-6" /></button>
        </div>
        <div className="p-6 space-y-4">
          <div>
            <label htmlFor="name" className={labelClass}>部署名 *</label>
