...
const InvoiceOCR: React.FC<InvoiceOCRProps> = ({ onSaveExpenses, addToast, requestConfirmation, isAIOff, accountItems, allocationDivisions }) => {
    ...
    const handleFileUpload = async (file: File) => {
        ...
        try {
            const tempItem: InboxItem = {
                ...
                status: 'processing', extractedData: null, errorMessage: null, createdAt: new Date().toISOString(),
            };
            setItems(prev => [tempItem, ...prev]);

            const { path } = await uploadFile(file, 'inbox');
            const newDbItem = await addInboxItem({ fileName: file.name, filePath: path, mimeType: file.type, status: 'processing', extractedData: null, errorMessage: null });
            tempId = newDbItem.id; 
            setItems(prev => prev.map(i => i.id.startsWith('temp-') ? { ...newDbItem, fileUrl: URL.createObjectURL(file) } : i));

            if (isAIOff) {
                await updateInboxItem(newDbItem.id, { status: 'pending_review', errorMessage: 'AI機能無効のため自動解析スキップ' });
                await loadItems();
                return;
            }
            ...
            await updateInboxItem(newDbItem.id, { status: 'pending_review', extractedData: ocrData });
            await loadItems();
        } catch (err: any) {
            setError(err.message || 'ファイルのアップロードまたは解析中にエラーが発生しました。');
            addToast('ファイル処理エラー', 'error');
            if (tempId && !tempId.startsWith('temp-')) {
                await updateInboxItem(tempId, { status: 'error', errorMessage: err.message });
            }
            await loadItems();
        }
    };
    ...
    const handleApprove = async (item: InboxItem) => {
        ...
        await updateInboxItem(item.id, { status: 'approved' });
        await loadItems();
    };
