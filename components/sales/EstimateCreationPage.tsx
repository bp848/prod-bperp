...
const EstimateCreationPage: React.FC<EstimateCreationPageProps> = ({ customers, currentUser, isAIOff, onCreateEstimate, onNavigateBack, addToast }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError('');
        try {
            const estimateData: Omit<Estimate, 'id' | 'createdAt' | 'updatedAt' | 'estimateNumber' | 'subtotal' | 'taxTotal' | 'grandTotal'> & { totalAmount: number } = {
                customerName,
                title,
                items: lineItems,
                deliveryDate,
                ...
                totalAmount: total, // FIX: Added totalAmount to match Estimate type requirement
            };
